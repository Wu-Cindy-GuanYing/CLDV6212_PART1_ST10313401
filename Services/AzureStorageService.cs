...
namespace ABCRetailers.Services
{
    public class AzureStorageService : IAzureStorageService
    {
        ...
        public async Task<T> UpdateEntityAsync<T>(T entity) where T : class, ITableEntity
        {
            ...
            {
                _logger.LogWarning(ex, "Error updatig entity {EntityType} with {RowKey}: {Message}",
        ...
        public async Task<byte[]> DownlaodFromFileShareAsync(string shareName, string fileName, string directoryName = "")
        {
            var shareClient = _shareServiceClient.GetShareClient(shareName);
            var directoryClient = string.IsNullOrEmpty (directoryName)
                ? shareClient.GetRootDirectoryClient()
